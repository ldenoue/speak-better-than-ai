// Adapted from Supertone's MIT-licensed Supertonic v3 browser example:
// https://github.com/supertone-inc/supertonic/blob/main/web/helper.js
import * as ort from 'onnxruntime-web';

const ASSET_CACHE='supertonic-v3-assets-3cadd1ee';
const pendingCacheWrites=new Set();

async function fetchCachedAsset(url){
    ...
    try{
        ...
        const write=cache.put(url,response.clone()).catch(error=>console.warn('[supertonic] Browser cache write failed; continuing without persistence.',error)).finally(()=>pendingCacheWrites.delete(write));
        pendingCacheWrites.add(write);
        return response;
    }catch(error){
        console.warn('[supertonic] Browser cache unavailable; using the network response.',error);
        return fetch(url);
    }
}
...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    ...
    const firstResponse = await fetchCachedAsset(voiceStylePaths[0]);
    ...
    // Fill in the data
    for (let i = 0; i < bsz; i++) {
        const response = await fetchCachedAsset(voiceStylePaths[i]);
...
export async function loadCfgs(onnxDir) {
    const response = await fetchCachedAsset(`${onnxDir}/tts.json`);
...
export async function loadTextProcessor(onnxDir) {
    const response = await fetchCachedAsset(`${onnxDir}/unicode_indexer.json`);
...
export async function loadOnnx(onnxPath, options) {
    const response = await fetchCachedAsset(onnxPath);
    if(!response.ok)throw new Error(`Could not load Supertonic asset: HTTP ${response.status}`);
